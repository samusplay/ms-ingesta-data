...
async def create_dataset(
    ...
    if extension.lower() not in [".csv", ".json"]:
        background_tasks.add_task(send_audit_event, trace_id, "FORMAT_ERROR", f"Formato inválido intentado: {extension}", status="FAILED")
        raise HTTPException(
            status_code=400, 
    ...
    if len(content) == 0:
        background_tasks.add_task(send_audit_event, trace_id, "EMPTY_FILE_ERROR", "Se intentó subir un archivo vacío.", status="FAILED")
        raise HTTPException(
            status_code=400, 
    ...
    try:
        ...
        background_tasks.add_task(send_audit_event, trace_id, "VALIDATION_FAILED", dve.message, status="FAILED")
        # CA 1: Retornar 422 Unprocessable Entity
        raise HTTPException(status_code=422, detail=dve.message)
        
    except ValueError as ve:
        background_tasks.add_task(send_audit_event, trace_id, "VALUE_ERROR", str(ve), status="FAILED")
