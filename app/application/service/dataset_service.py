...
class DatasetService:
    ...
    @staticmethod
    def min_max_scale_column(column: pd.Series) -> pd.Series:
        """Escala una columna numérica usando la fórmula Min-Max.

        La fórmula es:
            (valor - min) / (max - min)

        Si todos los valores son idénticos, se devuelve una serie de 0.0
        para evitar la división por cero.
        """
        numeric = pd.to_numeric(column, errors="coerce").astype(float)
        min_val = numeric.min()
        max_val = numeric.max()
        ...
        return ((numeric - min_val) / (max_val - min_val)).astype(float)
